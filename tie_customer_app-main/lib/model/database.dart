...
class Database{
  ...
  List<String> ser = ['REQUESTED', 'ACCEPTED', 'STARTED', 'FINISHED', 'PAST_ORDER'];
  ...
  Future<List<Category>> getCategories() async {
    ...
    for(var userData in data.docs){
      ...
      services.add(new Category(service['id'], service['title'], service['image'], service['description']));
    }
  ...
  Future<List<Customer>> getCustomers() async {
    ...
    for(var userData in customerData.docs){
      ...
      customers.add(new Customer(customer['id'], customer['name'], customer['email'], customer['image'], customer['address']));
    }
  ...
  Future<Customer> getCustomerByEmail() async {
    customers = <Customer>[];
    QuerySnapshot customerData = await db.collection('Customers').where('email',isEqualTo: _auth.currentUser.email).get();
    if(customerData.size == 1){
      Map customer = customerData.docs[0].data();
      return new Customer(customer['id'], customer['name'], customer['email'], customer['image'], customer['address']);
    }
  }
  ...
  Future<List<ServiceProvider>> getServiceProviders() async {
    ...
    QuerySnapshot serviceProviderData = await db.collection('ServiceProviders').get();
    for(var userData in serviceProviderData.docs){
      Map serviceProvider = userData.data();
      employees.add(new ServiceProvider(serviceProvider['id'], serviceProvider['name'], serviceProvider['email'], serviceProvider['image'], serviceProvider['address'], serviceProvider['fees'], serviceProvider['services'], serviceProvider['rating'], serviceProvider['numOfOrders']));
    }
  ...
  Future<List<OrderItem>> getOrders() async {
    ...
    for(var userData in orderData.docs){
      ...
      orders.add(new OrderItem(order['orderID'], new Customer(order['buyer']['id'], order['buyer']['name'],
          order['buyer']['email'], order['buyer']['image'], order['buyer']['address']),
          new ServiceProvider(order['seller']['id'], order['seller']['name'], order['seller']['email'],
              order['seller']['image'], order['seller']['address'], order['seller']['fees'],
              order['seller']['services'], order['seller']['rating'], order['seller']['numOfOrders']), order['startTime'], order['endTime'],
          new Category(order['category']['id'], order['category']['title'], order['category']['image'],
              order['category']['description']), order['rating'],
              getOrderStatus(order['orderStatus']), order['address'], order['description']));
    }
  ...
  Future<void> updateCustomer(Customer customer) async {
    QuerySnapshot query = await db.collection("Customers").where('email', isEqualTo: customer.email).get();
    if(query.docs[0].data()['image'] != null){
      if(customer.imageUrl != "")
        customer.imageUrl = await uploadImage(new File(customer.imageUrl));
    }
    await db.collection('Customers').doc(query.docs[0].id).update(customer.asMap());
  }
  ...
  Future<void> updateServiceProvider(ServiceProvider serviceProvider) async {
    QuerySnapshot query = await db.collection("ServiceProviders").where('id', isEqualTo: serviceProvider.id).get();
    await db.collection('ServiceProviders').doc(query.docs[0].id).update(serviceProvider.asMap());
  }
